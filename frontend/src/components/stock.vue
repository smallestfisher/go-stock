...
function normalizeStockCode(code) {
  ...
  const match = value.match(/^([0-9]{5,6})\.(sh|sz|bj)$/)
  if (match) {
...
function normalizeFollowedStockCode(code) {
  const value = String(code || '').trim()
  if (value.startsWith("us")) {
    return "gb_" + value.replace("us", "").toLowerCase()
  }
  return normalizeStockCode(value)
}

function hasStockCode(code) {
  const normalized = normalizeStockCode(code)
  return stocks.value.some(item => normalizeStockCode(item) === normalized)
}
...
const groupResults = computed(() => {
  ...
  if (currentGroupId.value === 0) {
    ...
    for (const key in sortedResults.value) {
      if (hasStockCode(sortedResults.value[key]['股票代码'])) {
...
function connectDanmakuWebSocket() {
  if (!data.enableDanmu || ws.value) return
...
onBeforeMount(() => {
  ...
  EventsOn("newChatStream", async (msg) => {
    ...
    if (msg === "DONE") {
      ...
      message.destroyAll()
      if (data.airesult && !aiAnalysisSaved.value) {
        aiAnalysisSaved.value = true
        await SaveAIResponseResult(data.code, data.name, data.airesult, data.chatId, data.question, data.aiConfigId)
          .catch(err => {
            const errMsg = err?.message || err || "未知错误"
            message.error("保存AI分析结果失败: " + errMsg)
          })
      }
      data.loading = false
...
onMounted(() => {
  ...
  GetFollowList(currentGroupId.value).then(result => {
    ...
    for (const followedStock of result) {
      const stockCode = normalizeFollowedStockCode(followedStock.StockCode)
      if (!hasStockCode(stockCode)) {
        stocks.value.push(stockCode)
      }
      Greet(stockCode).then(result => {
        updateData(result)
      })
    }
    //monitor()
...
function AddStock() {
  ...
  if (!hasStockCode(data.code)) {
    return Follow(data.code).then(result => {
      if (result === "关注成功") {
        stocks.value.push(normalizeFollowedStockCode(data.code))
...
function removeMonitor(code, name, key) {
  ...
  const index = stocks.value.findIndex(item => normalizeStockCode(item) === normalizeStockCode(code))
  if (index >= 0) {
...
/** 新浪/应用内代码转为东方财富接口常用格式（如 600519.SH）—— 实现见 ../utils/stockCode */
...
async function showLightweightKline(code, name) {
  ...
  await prepareStockTradingPrice(code, name)
...
async function prepareStockTradingPrice(code, name) {
  ...
  try {
    ...
    console.error('[prepareStockTradingPrice] 刷新自选列表失败:', e)
  }
...
const cardActions = {
  ...
  aiCheckStock,
  delStockGroup,
  showLightweightKline,
  showFenshi,
  setStock,
  getStockMobileMoreOptions,
  handleStockMobileMoreSelect,
  showK,
  showMoney,
  search,
  searchNotice,
...
function aiReCheckStock(stock, stockCode) {
  if (aiAnalysisInProgress.value) {
    message.warning("当前AI分析还在进行中，请等待完成后再开始新的分析")
    return
  }
  data.name = stock
  data.code = stockCode
  ...
  aiAnalysisTimeout.value = setTimeout(() => {
    if (aiAnalysisInProgress.value && data.loading) {
      data.loading = false
      data.analysisStatus = ""
      aiAnalysisInProgress.value = false
      aiAnalysisSaved.value = false
...
function aiCheckStock(stock, stockCode) {
  if (aiAnalysisInProgress.value) {
    modalShow4.value = true
    message.warning("当前AI分析还在进行中，请等待完成后再切换股票")
    return
  }
...
<template>
  <vue-danmaku v-if="data.enableDanmu" v-model:danmus="danmus" useSlot
  ...
  <n-tabs class="stock-page-shell" type="card" style="" animated addable :data-currentGroupId="currentGroupId"
    ...
    <n-tab-pane closable name="0" :tab="'全部'">
      <n-grid class="stock-card-grid" :x-gap="8" :cols="3" :y-gap="8">
        <n-gi v-for="result in sortedResults" :key="result['股票代码']" :id="result['股票代码'] + '_gi'" style="margin-left: 2px;">
    ...
    <n-tab-pane closable v-for="group in groupList" :group-id="group.ID" :name="String(group.ID)" :tab="group.name">
      <n-grid class="stock-card-grid" :x-gap="8" :cols="3" :y-gap="8">
        <n-gi v-for="result in groupResults" :key="result['股票代码']" :id="result['股票代码'] + '_gi'" style="margin-left: 2px;">
          <n-card :data-sort="result.sort" :id="result['股票代码']" :data-code="result['股票代码']" :bordered="true"
                  :title="result['股票名称']" :closable="false"
                  @close="removeMonitor(result['股票代码'],result['股票名称'],result.key)">
        ...
        <n-flex class="stock-ai-modal__actions" justify="right">
          ...
          <n-flex class="stock-ai-modal__bottom-actions" justify="right">
            ...
            <n-button
                size="tiny"
                type="warning"
                :loading="aiAnalysisInProgress"
                :disabled="aiAnalysisInProgress"
                @click="aiReCheckStock(data.name,data.code)"
            >
              开始AI分析
            </n-button>
