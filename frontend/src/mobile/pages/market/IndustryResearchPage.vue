...
import { GetStockList, IndustryResearchReport } from '../../../api/app'
...
async function loadReports(code = '') {
  loading.value = true
  try {
    const res = await IndustryResearchReport(code)
    list.value = pickArray(res)
  } catch (e) {
    console.error('加载行业研究失败:', e)
    list.value = []
  } finally {
    loading.value = false
  }
        ...
        <template v-else>
          <div v-if="list.length" class="report-list">
            <div v-for="(item, i) in list" :key="item.infoCode || i" class="report-item">
              <!-- 行业标签 + 标题（可点开 PDF） -->
              <div class="title-row">
                <span v-if="item.industryName" class="industry-tag">{{ item.industryName }}</span>
