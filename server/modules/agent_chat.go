...
func init() {
	...
	server.Register("ChatWithAgent", func(_ context.Context, core *server.Core, args server.Args) (any, error) {
		...
		go func() {
			...
			for msg := range ch {
				core.Events.Emit("agent-message", agentMessageToMap(msg))
			}
			core.Events.Emit("agent-message", agentMessageToMap(&schema.Message{
				Role:    schema.Assistant,
				Content: "agent-DONE",
			}))
			core.SetAgentCancel(nil)
		}()
		return nil, nil
	})
}
